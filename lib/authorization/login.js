 ...
 */
module.exports = function(thorin, storeObj, opt, historyObj) {
    ...
    .use((intentObj, next) => {
      ...
      AccountModel.find(qry).then((accObj) => {
        ...
        bcrypt.compare(password, accPassword, (err, isMatch) => {
          ...
          if(historyObj) {
            historyObj.create(accObj, 'LOGIN', intentObj).catch((err) => {
              logger.error(`Failed to create login history entry for user ${identity}`, err);
            });
          }
