...
module.exports = function(thorin, storeObj, opt) {
  ...
  }).error('AUTH.PASSWORD.INVALID', 'Please enter your old password', 400);
  // Add the "password_new" field.
  inputData[passField + '_new'] = dispatcher.validate("STRING", {
    max: MAX_PASSWORD_LENGTH
  }).error('AUTH.PASSWORD.INVALID', 'Please enter your new password', 400);
  // Add the "password_check" field.
  inputData[passField + '_check'] = dispatcher.validate("STRING", {
    max: MAX_PASSWORD_LENGTH
  }).error('AUTH.PASSWORD.INVALID', 'Passwords do not match', 400);
  ...
  dispatcher
    ...
    .use((intentObj, next) => {
      ...
      if (!accountObj) {
        ...
        return next(thorin.error('AUTH.PASSWORD.INVALID', 'Please login to continue', 403), thorin.error('AUTH.PASSWORD', 'Missing account model instance in intent data'));
      }
      ...
      if (newPass !== checkPass) {
        return next(thorin.error('AUTH.PASSWORD.INVALID', 'Passwords do not match.'));
      }
      ...
      hasher.compare(oldPass, accountObj.get(passField), (err, isMatch) => {
        if(err) {
          logger.warn(`Failed to hash password for pass change authorization`);
          return next(thorin.error('AUTH.PASSWORD.ERROR', 'An error occurred. Please try again.', err, 500));
        }
        if(!isMatch) {
          return next(thorin.error('AUTH.PASSWORD.INVALID', 'Invalid password.', 403));
        }
        // at this point, we generate the new hash.
        hasher.hash(newPass, 10, (err, newHash) => {
          if(err) {
            logger.error('Failed to generate new password hash for pass change.');
            return thorin.error('AUTH.PASSWORD.ERROR', 'An error occurred. Please try again', 500, err);
          }
